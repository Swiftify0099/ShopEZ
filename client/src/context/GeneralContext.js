...
const GeneralContextProvider = ({ children }) => {
  ...
  const fetchCartCount = async () => {
    ...
    if (userId) {
      await axios.get('https://shopez-cucq.onrender.com/fetch-cart').then(
        (response) => {
  ...
  const login = async () => {
    try {
      ...
      await axios.post('https://shopez-cucq.onrender.com/login', loginInputs)
        .then(async (res) => {
  ...
  const register = async () => {
    try {
      await axios.post('https://shopez-cucq.onrender.com/register', inputs)
        .then(async (res) => {
          localStorage.setItem('userId', res.data._id);
          localStorage.setItem('userType', res.data.usertype);
          localStorage.setItem('username', res.data.username);
